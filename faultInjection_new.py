...
def main():
    ...
    for i in range(1,100):
	    ...
	    hdlr = logging.FileHandler(configure.benchmark+"_"+configure.kernel+'.log'+str(i))
	    formatter = logging.Formatter("%(levelname)s %(message)s")
	    hdlr.setFormatter(formatter)
	    logger.addHandler(hdlr)
	    logger.setLevel(logging.INFO)
	    processLog(configure.profile_file+"_"+str(i))
            
	    for trial in range(1500):# Jiesheng: what's the meaning of 3000 here
