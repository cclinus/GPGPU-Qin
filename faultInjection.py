...
#    cuda_gdb_p.expect(CUDA_GDB_EXPECT)
    ...
    # JUSTIN FIX
    if "x" in basepcline:
	print "Hex PC is wrong 1 !"
	logger.info("Hex PC is wrong 1 !")
	#return -1
    basepcline = basepcline.lstrip(" ").rstrip("\r\n")
    basepcline_list = basepcline.split(" ")
    basepc = basepcline_list[len(basepcline_list)-1]
    #JUSTIN FIX
    if int(str(basepc), 16) == False:
	print "Hex PC is wrong 2 !"
	logger.info("Hex PC is wrong 2 !")
	basepc = basepcline.split()[-1]
	#return -1
    ...
    # JUSTIN: FIX
    if "Invalid coordinates." in cuda_gdb_p.before:
    	logger.info("JUSTIN: MEET 'Invalid coordinates', exit this trial! ")
	return -1
    # JUSTIN ENDs
        ...
        if i == 0:
            ...
            if CUDA_EXCEPTION in logs:
                ...
                if "PASS" in logs:
                        ...
                        logger.info("At trial "+str(trial)+" fault in register "+reg+" executed unknown")
                        logger.info(logs)  
...
# Jiesheng: not understand
        ...
        logger.info("\n\n\n********************************************************************\nTrial "+str(trial)+" starts!\n")
        ft_start = datetime.now()
        returnCode = faultMain(configure.binary_path,breakpoint,trial,pc,str(profile.keys()[kernel]),iteration)
	...
	#os.system("mv result.txt result.txt-" + `trail`)
        time.sleep(5)

    logger.info("Total Success Run: " + `successCounter`)
